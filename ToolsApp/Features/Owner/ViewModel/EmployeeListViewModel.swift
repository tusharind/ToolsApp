...
final class EmployeeListViewModel: ObservableObject {
    ...
    func fetchEmployees() async {
        ...
        do {
            let response: EmployeeResponse = try await APIClient.shared.send(request, responseType: EmployeeResponse.self)
    ...
    func fetchFactories() async {
        let request = APIRequest(path: "/owner/factories?size=100", method: .GET)
        
        do {
            let response = try await APIClient.shared.send(request, responseType: APIResponse<FactoryListResponse>.self)
    ...
    // MARK: - Filtered Factories
    var filteredFactories: [Factory] {
        if factorySearchText.isEmpty { return factories }
        return factories.filter { $0.name.lowercased().contains(factorySearchText.lowercased()) }
    }
}
