...
final class CentralOfficerViewModel: ObservableObject {
    ...
    // MARK: - Fetch all offices
    func fetchOffices() async {
        isLoading = true
        errorMessage = nil
        
        let request = APIRequest(
            path: "/owner/central-office",
            method: .GET,
            parameters: nil,
            headers: nil,
            body: nil
    ...
    func addOfficer(to officeId: Int, name: String, email: String, phone: String?) async {
        ...
        do {
            ...
            if response.success {
                // Refresh offices to include the new officer
                await fetchOffices()
            } else {
                self.errorMessage = response.message
            }
    ...
    // MARK: - Filter officers by search & role
